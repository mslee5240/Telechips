...
void init_button(void) {
	// GPIOC의 Pin 0, 1, 2, 3을 Pull-up으로 Set
	*(unsigned int*)GPIOC_PUPDR = 0x00000055;
}
...
int get_button(GPIO_TypeDef* GPIO, int GPIO_Pin, int button_num)
{
    ...
    // 버튼 상태 읽기; [HAL 방식]
//    int current_state = HAL_GPIO_ReadPin(GPIO, GPIO_Pin);

    // 방법_1; [DMA 방식으로 대체]
    int current_state = (*(unsigned int*)GPIOC_IDR & GPIO_Pin) ? BUTTON_RELEASE : BUTTON_PRESS;

    // 방법_2;
//    int current_state = (*(unsigned int*)GPIOC_IDR & GPIO_Pin) >> button_num;
