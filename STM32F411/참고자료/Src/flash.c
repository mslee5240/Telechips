...
Sector 7    0x8060000-0x807FFFF          128K bytes <- user data
...
t_student student;					// 아직 RAM에 저장되어있음
t_set_time set_time;
...
printf("flash EMPTY !!!!\n");
		...
		student.grade=4.0;
		printf("w grade: %lf\n", student.grade);
		flash_write((uint32_t *) &student, sizeof(student));		// <- flash에 write를 해줘야 flash에 작성됨
	}
