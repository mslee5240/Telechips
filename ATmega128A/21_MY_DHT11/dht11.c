...
int us_count = 0;
volatile enum state_define dht11_state = OK;
...
enum state_define dht11_main(uint8_t* data)
{
	...
	memset(data, 0, 5);	// 데이터 배열 초기화
	dht11_state = OK;
		
	// ===== [Step 1: Start Signal] ===== 
	init_dht11();		// 데이터 라인을 HIGH로 초기화 (풀업 상태)
	_delay_ms(100);		// 안정화를 위해 대기
		
	DHT_PORT &= ~(1 << DHT_PIN_NUM);	// 데이터 라인을 LOW로 내려서 시작 신호 전송
	_delay_ms(20);						// 최소 datasheet 권장 18ms 이상 LOW 유지 (여기서는 20ms)
	...
	if (dht11_state == OK)
	{
		dht11_state = pin_state_check(HIGH, 100);
	}
	...
	{	
		...
		{	
			...
			{	
				...
				dht11_state = pin_state_check(LOW, 70);
				if (dht11_state == TIMEOUT) {
					i = 5;
					j = -1;
				}
				...
				if (dht11_state == OK)
				{
					//dht11_state = pin_state_check(HIGH, 90);
					//if (dht11_state == TIMEOUT) {
						//i = 5;
						//j = -1;
					//}
		...
		_delay_us(50);	// 데이터 전송 후 약간의 여유 시간 (spec: 50us, 여기서는 60us 여유)
	}
	return dht11_state;
}
	

enum state_define pin_state_check(uint8_t pin_state, uint32_t delay_time)
{
	us_count = 0;
	while ( ( (DHT_PIN & 1 << DHT_PIN_NUM) >> DHT_PIN_NUM) == pin_state)	// 특정 데이터 핀의 상태가 HIGH인지 확인하는 역할
	{
		_delay_us(2);
		us_count += 2;
		if (us_count > delay_time)	// 50us 이상 HIGH가 지속되면 타임아웃 발생
		{
			return TIMEOUT;
		}
	}
